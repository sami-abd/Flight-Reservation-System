...
app.post("/api/v1/user/createUser/", (req, res) => {
  try {
    ...
    // Define SQL query:
    const query =
      "INSERT INTO USER (firstName, lastName, birthdate, email, street, city, province, password, userType) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'REGISTERED_USER')";

    // Run SQL query with provided value:
    db.query(
      query,
      [firstName, lastName, birthdate, email, street, city, province, password],
      (error, results) => {
        // Handle for when no results are returned
        if (error) {
          console.log(error);
